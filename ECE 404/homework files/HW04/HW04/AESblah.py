...
class AES():
    ...
    def encrypt(self, plaintext:str, ciphertext:str) -> None:
        # mixcole = [[0x2, 0x3, 0x1, 0x1],
        #            [0x1, 0x2, 0x3, 0x1],
        #            [0x1, 0x1, 0x2, 0x3],
        #            [0x3, 0x1, 0x1, 0x2]]
        # mixbv = BitVector(intVal = mixcole, size = 8)
        key = BitVector(textstring = self.key)
        ...
        while(bitvector.more_to_read):
            ...
            round = 1
            for round_key in key_schedule[1:15]:
                statearray = [[0 for x in range(4)] for x in range(4)]
                for i in range(4):
                    for j in range(4):
                ...
                for i in range(4):
                    for i in range(4):
                        statearray[i][j] = BitVector(intVal=self.subBytesTable[statearray[i][j].intValue()], size = 8)
                # if round == 1:
                #     for i in range(4):
                #         for j in range(4):
                ...
                #     mixed_statearray = BitVector(intVal = mixed_statearray)
                #     for i in range(4):
                #         for j in range(4):
                #             for x in range(4):
                #                 mixed_statearray ^= mixbv[i][x].gf_multiply_modular(new_statearray[x][j], self.AES_modulus, 8)
                # else:
                #     mixed_statearray = new_statearray
                mixed_statearray = self.ColumnMix(new_statearray)
                bitvec = BitVector(size=0)
                for j in range(4):
                    for i in range(4):
                        bitvec += mixed_statearray[i][j]
                bitvec ^= round_key
                round += 1
                
            cipher.write(bitvec.get_bitvector_in_hex())
        bitvector.close_file_object()
        cipher.close()
    '''
